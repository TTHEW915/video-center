...
export default function Dashboard() {
    ...
    return (
      <div style={{minHeight:'100vh',background:'#080809',display:'flex',alignItems:'center',justifyContent:'center',fontFamily:"'DM Mono',monospace"}}>
        <div style={{textAlign:'center'}}>
          <div style={{color:'#63ffb4',fontSize:13,marginBottom:16}}>Loading...</div>
          <button onClick={() => router.push('/')} style={{background:'none',border:'none',color:'#333',fontSize:11,cursor:'pointer',fontFamily:"'DM Mono',monospace"}}>
            ← Back to login
          </button>
        </div>
      </div>
  ...
  return (
    <div style={{minHeight:'100vh',background:'#080809',fontFamily:"'DM Mono',monospace",padding:'40px 24px 60px',display:'flex',flexDirection:'column',alignItems:'center'}}>
      ...
      {!accessToken && (
        <div style={{width:'100%',maxWidth:860,marginBottom:16,background:'rgba(255,180,50,0.05)',border:'1px solid rgba(255,180,50,0.3)',borderRadius:12,padding:'14px 18px',color:'#ffb432',fontSize:12}}>
          ⚠️ YouTube not connected. Please{' '}
          <button onClick={signOut} style={{background:'none',border:'none',color:'#ffb432',textDecoration:'underline',cursor:'pointer',fontSize:12,fontFamily:"'DM Mono',monospace"}}>sign out</button>
          {' '}and sign in again — click <strong>Allow</strong> when Google asks for YouTube permissions.
        </div>
      )}
      ...
      <div style={{width:'100%',maxWidth:860,display:'flex',flexDirection:'column',gap:20}}>
        ...
        <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(160px,1fr))',gap:12}}>
          {[
            {name:'YouTube',icon:'▶',color:'#FF0000',bg:'#1a0000',connected:!!accessToken},
            ...
            <div key={p.name} style={{background:p.connected?p.bg:'#0e0e10',border:`1.5px solid ${p.connected?p.color+'55':'#1e1e21'}`,borderRadius:14,padding:'16px 18px',opacity:p.connected?1:0.5}}>
              ...
              {p.connected ? (
                ...
                <div style={{color:'#444',fontSize:9}}>{p.name==='YouTube'?'Sign in to connect':'Coming soon'}</div>
              )}
            </div>
          ))}
        </div>
        ...
        {error && (
          <div style={{background:'rgba(255,50,50,0.05)',border:'1px solid rgba(255,50,50,0.2)',borderRadius:12,padding:'16px 20px',color:'#ff6666',fontSize:12,lineHeight:1.6}}>
            {error}
          </div>
        )}
